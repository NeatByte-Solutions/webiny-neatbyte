import Link from 'next/link'
import clsx from 'clsx'

export function Footer({ previous, next }) {
  return (
    <footer className="text-sm leading-6 my-[1.875rem] md:my-[3.75rem]">
      {(previous || next) && (
        <div className="text-slate-700 font-semibold grid grid-cols-2 content-center dark:text-slate-200 w-full border-t border-b border-neutral-200 pt-2 md:pt-[1.4375rem] pb-2 md:pb-[1.8125rem]">
          {previous ? (
            <Link href={previous.href}>
              <a
                className={`group flex items-center justify-start md:justify-end hover:text-slate-900 dark:hover:text-white pr-[2.1875rem] border-r border-neutral-200`}
              >
                <img
                  src="data:image/svg+xml;base64,"
                  className="mr-3"
                  alt="arrow"
                />
                {previous.shortTitle || previous.title}
              </a>
            </Link>
          ) : (
            <div />
          )}
          {next ? (
            <Link href={next.href}>
              <a className="group flex items-center justify-end md:justify-start hover:text-slate-900 dark:hover:text-white pl-[2.1875rem] border-l border-neutral-200">
                {next.shortTitle || next.title}

                <img
                  src="data:image/svg+xml;base64,"
                  className="ml-3"
                  alt="arrow"
                />
              </a>
            </Link>
          ) : (
            <div />
          )}
        </div>
      )}
      <div className="pt-[1.875rem] grid md:grid-cols-2 text-slate-500 gap-y-[1.875rem] md:gap-y-0 md:gap-x-10">
        <div className="grid gap-y-1.5">
          <div className="flex">
            <img
              src="data:image/svg+xml;base64,"
              className="mr-2.5 h-6 w-1.5"
              alt="arrow"
            />
            <p>
              Questions? <a href="#">Find us on slack.</a>
            </p>
          </div>
          <div className="flex">
            <img
              src="data:image/svg+xml;base64,"
              className="mr-2.5 h-6 w-1.5"
              alt="arrow"
            />
            <p>
              Found a bug on the page, <a href="#">submit an issue or a PR.</a>
            </p>
          </div>
          <div className="flex">
            <img
              src="data:image/svg+xml;base64,"
              className="mr-2.5 h-6 w-1.5"
              alt="arrow"
            />
            <p>
              Check out our <a href="#">roadmap.</a>
            </p>
          </div>
        </div>
        <div>
          <p>Join our developer newsletter</p>
          <div className="bg-black mt-2.5 mb-[0.3125rem] h-10 w-full" />
          <p>You can unsuscribe at any time.</p>
        </div>
      </div>
    </footer>
  )
}
